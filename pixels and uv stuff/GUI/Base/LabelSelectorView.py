...
from PyQt5.QtWidgets import QWidget, QColorDialog, QComboBox, QLineEdit
...
class _LabelUI(QWidget, Ui_LabelSelector):
...
class LabelSelectorView(CustomDockWidget):
    new_label_added = pyqtSignal(str)
    drop_down_item_selected = pyqtSignal(str)
    edited_label = pyqtSignal(str)
    ...
    def __init__(self) -> None:
        ...
        self.selector = _LabelUI()
        self.vertical_layout.insertWidget(0, self.selector)
        # signal setup
        # propagate signal up

        self.selector.new_label_added.connect(self.new_label_added)
        self.selector.drop_down_item_selected.connect(self.drop_down_item_selected)
        self.selector.edited_label.connect(self.edited_label)

        # self.selector.new_label_added.connect(self.handle_new_label_added)

    def add_item_to_vertical_layout(self, item):
        pass

    def handle_new_label_added(self, text):
        self.selector.add_new_item(text)

    def change_current_label(self, text):
        self.selector.set_current_label(text)

    def edit_label(self, text):
        self.selector.update_dropdown_item_text(text)

    def reset_current_label(self):
        text = self.selector.dropdown.currentText()
        # block signal first
        self.selector.set_name_input(text)